...
namespace Prime31
{
	public class EtceteraUIManager : MonoBehaviourGUI
	{
		...
		private void OnGUI()
		{
			...
			if (GUILayout.Button("Share Resized Image Natively"))
			{
			...
			if (GUILayout.Button("Prompt to Take Photo Applying EXIF"))
			{
				EtceteraAndroid.promptToTakePhoto("photo.jpg", insertIntoMediaStore: false, applyExifOrientation: true);
			}
		...
		{
			EtceteraAndroid.scaleImageAtPath(imagePath, 0.3f);
