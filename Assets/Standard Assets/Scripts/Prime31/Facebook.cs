...
namespace Prime31
{
	public class Facebook : P31RestKit
	{
		...
		public IEnumerator fetchImageAtUrl(string url, Action<Texture2D> completionHandler)
		{
			...
			{
				UnityEngine.Debug.Log("Error attempting to load profile image: " + www.error);
				completionHandler?.Invoke(null);
			}
			else
			{
				completionHandler?.Invoke(www.texture);
			}
		}
		...
		public void checkSessionValidityOnServer(Action<bool> completionHandler)
		{
			get("me", delegate(string error, object obj)
			{
				...
				{
					completionHandler(obj: true);
				}
				else
				{
					completionHandler(obj: false);
				}
			});
		}
				...
				if (error == null && obj != null && obj is IDictionary)
				{
					...
					try
					{
						while (enumerator.MoveNext())
						{
							...
							if (dictionary2["status"].ToString() == "granted")
							{
								list2.Add(dictionary2["permission"].ToString());
							}
						}
					}
					finally
		...
		public void postScore(int score, Action<bool> completionHandler)
		{
			...
			post("me/scores", parameters, delegate(string error, object obj)
			{
				...
				else if (error == null && obj != null && obj is IDictionary)
				{
					IDictionary dictionary2 = obj as IDictionary;
					bool obj2 = (bool)dictionary2["success"];
