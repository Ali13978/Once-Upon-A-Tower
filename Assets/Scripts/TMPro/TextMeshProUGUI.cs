	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				vector.x = m_xAdvance + (m_cached_TextElement.xOffset - num5 - num6) * num2 * (1f - m_charWidthAdjDelta);
				vector.y = (baseline + m_cached_TextElement.yOffset + num5) * num2 - m_lineOffset + m_baselineOffset;
				vector.z = 0f;
				vector2.x = vector.x;
				vector2.y = vector.y - (m_cached_TextElement.height + num5 * 2f) * num2;
				vector2.z = 0f;
				vector3.x = vector2.x + (m_cached_TextElement.width + num5 * 2f + num6 * 2f) * num2 * (1f - m_charWidthAdjDelta);
				vector3.y = vector.y;
				vector3.z = 0f;
				vector4.x = vector3.x;
				vector4.y = vector2.y;
			...
			case TextAlignmentOptions.Justified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? ((rectTransformCorners[0] + rectTransformCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_textInfo.pageInfo[num10].ascender + margin.y + m_textInfo.pageInfo[num10].descender - margin.w) / 2f, 0f)) : ((rectTransformCorners[0] + rectTransformCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_maxAscender + margin.y + num14 - margin.w) / 2f, 0f)));
				break;
			case TextAlignmentOptions.BottomLeft:
			case TextAlignmentOptions.Bottom:
			case TextAlignmentOptions.BottomRight:
			case TextAlignmentOptions.BottomJustified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (rectTransformCorners[0] + new Vector3(margin.x, 0f - m_textInfo.pageInfo[num10].descender + margin.w, 0f)) : (rectTransformCorners[0] + new Vector3(margin.x, 0f - num14 + margin.w, 0f)));
				break;
