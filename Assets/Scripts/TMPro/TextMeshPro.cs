...
namespace TMPro
{
	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		...
		protected override void Awake()
		{
			...
			{
				LocalizeUtil.LocalizeFont(this);
			}
		...
		protected override int SetArraySizes(int[] chars)
		{
			...
			for (int i = 0; chars[i] != 0; i++)
			{
				...
				if (!m_currentFontAsset.characterDictionary.TryGetValue(num2, out TMP_Glyph value))
				{
			...
			for (int m = 0; m < num4; m++)
			{
				...
				if (m_textInfo.meshInfo[m].vertices == null || m_textInfo.meshInfo[m].vertices.Length < referenceCount * (m_isVolumetricText ? 8 : 4))
				{
					if (m_textInfo.meshInfo[m].vertices == null)
					{
						if (m == 0)
						{
							m_textInfo.meshInfo[m] = new TMP_MeshInfo(m_mesh, referenceCount + 1, m_isVolumetricText);
						}
						else
						{
							m_textInfo.meshInfo[m] = new TMP_MeshInfo(m_subTextObjects[m].mesh, referenceCount + 1, m_isVolumetricText);
						}
					}
					else
					{
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				if (m_textElementType == TMP_TextElementType.Character && !isUsingAlternateTypeface && ((m_style & FontStyles.Italic) == FontStyles.Italic || (m_fontStyle & FontStyles.Italic) == FontStyles.Italic))
				{
					...
					Vector3 b = new Vector3(num20 * ((m_cached_TextElement.yOffset + num5 + num6) * num2), 0f, 0f);
					Vector3 b2 = new Vector3(num20 * ((m_cached_TextElement.yOffset - m_cached_TextElement.height - num5 - num6) * num2), 0f, 0f);
					vector += b;
					vector2 += b2;
					vector3 += b;
					vector4 += b2;
				...
				m_textInfo.characterInfo[m_characterCount].aspectRatio = (vector3.x - vector2.x) / (vector.y - vector2.y);
			...
			case TextAlignmentOptions.TopJustified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (textContainerLocalCorners[1] + new Vector3(margin.x, 0f - m_textInfo.pageInfo[num10].ascender - margin.y, 0f)) : (textContainerLocalCorners[1] + new Vector3(margin.x, 0f - m_maxAscender - margin.y, 0f)));
				break;
			case TextAlignmentOptions.Left:
			case TextAlignmentOptions.Center:
			case TextAlignmentOptions.Right:
			case TextAlignmentOptions.Justified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? ((textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_textInfo.pageInfo[num10].ascender + margin.y + m_textInfo.pageInfo[num10].descender - margin.w) / 2f, 0f)) : ((textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_maxAscender + margin.y + num14 - margin.w) / 2f, 0f)));
				break;
			case TextAlignmentOptions.BottomLeft:
			case TextAlignmentOptions.Bottom:
			case TextAlignmentOptions.BottomRight:
			case TextAlignmentOptions.BottomJustified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (textContainerLocalCorners[0] + new Vector3(margin.x, 0f - m_textInfo.pageInfo[num10].descender + margin.w, 0f)) : (textContainerLocalCorners[0] + new Vector3(margin.x, 0f - num14 + margin.w, 0f)));
				break;
			...
			case TextAlignmentOptions.CaplineJustified:
				a = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(margin.x, 0f - (m_maxCapHeight - margin.y - margin.w) / 2f, 0f);
				break;
			}
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.CaplineJustified:
				{
					...
					else if (character2 == '\t' || char.IsSeparator(character2))
					{
						...
						vector5 = (m_isRightToLeft ? (vector5 - new Vector3(num50 * (1f - num51) / (float)num52, 0f, 0f)) : (vector5 + new Vector3(num50 * (1f - num51) / (float)num52, 0f, 0f)));
					}
					else
					{
						vector5 = (m_isRightToLeft ? (vector5 - new Vector3(num50 * num51 / (float)(tMP_LineInfo.visibleCharacterCount - 1), 0f, 0f)) : (vector5 + new Vector3(num50 * num51 / (float)(tMP_LineInfo.visibleCharacterCount - 1), 0f, 0f)));
					}
					break;
				}
				}
