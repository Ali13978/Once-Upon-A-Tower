...
namespace Rewired.Data
{
	public class UserDataStore_PlayerPrefs : UserDataStore
	{
						...
						if (data.ContainsPlayer(allPlayer3.id))
						{
							...
							for (int j = 0; j < playerInfo2.joystickCount; j++)
							{
								...
								if (joystickInfo != null)
								{
									...
									{
										if (!TryFindJoysticksImprecise(joystickInfo, out List<Joystick> matches))
										{
											continue;
										}
		...
		{
			string controllerMapPlayerPrefsKey = GetControllerMapPlayerPrefsKey(player, saveData.controller, saveData.categoryId, saveData.layoutId, includeDuplicateIndex: true);
			PlayerPrefs.SetString(controllerMapPlayerPrefsKey, saveData.map.ToXmlString());
			controllerMapPlayerPrefsKey = GetControllerMapKnownActionIdsPlayerPrefsKey(player, saveData.controller, saveData.categoryId, saveData.layoutId, includeDuplicateIndex: true);
		...
		private string GetControllerMapPlayerPrefsKey(Player player, Controller controller, int categoryId, int layoutId, bool includeDuplicateIndex)
		{
			...
			if (controller.type == ControllerType.Joystick)
			{
				...
				{
					basePlayerPrefsKey = basePlayerPrefsKey + "|duplicate=" + GetDuplicateIndex(player, controller).ToString();
				}
			}
		...
		private List<int> GetControllerMapKnownActionIds(Player player, Controller controller, int categoryId, int layoutId)
		{
			...
			{
				if (!string.IsNullOrEmpty(array[i]) && int.TryParse(array[i], out int result))
				{
					list.Add(result);
				}
			}
