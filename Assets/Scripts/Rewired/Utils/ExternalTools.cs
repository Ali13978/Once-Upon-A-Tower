	...
	public class ExternalTools : IExternalTools
	{
				...
				if (GetAndroidAPILevel() >= 19)
				{
					...
					if (array != null)
					{
						for (int i = 0; i < array.Length; i++)
						{
							try
							{
								using (AndroidJavaObject androidJavaObject2 = androidJavaClass.CallStatic<AndroidJavaObject>("getDevice", new object[1]
								{
									array[i]
								}))
								{
		...
		public int GetAndroidAPILevel()
		{
			try
			{
				...
				{
					result = androidJavaClass.GetStatic<int>("SDK_INT");
				}
				return result;
			}
			catch
