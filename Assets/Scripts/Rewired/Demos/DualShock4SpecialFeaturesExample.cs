		...
		private void HandleTouchpad(IDualShock4Extension ds4)
		{
			...
			for (int i = 0; i < ds4.maxTouches; i++)
			{
				if (ds4.IsTouching(i))
				{
					...
					Transform transform = touch2.go.transform;
					float x2 = position.x - 0.5f;
					Vector3 localScale = touch2.go.transform.localScale;
					transform.localPosition = new Vector3(x2, 0.5f + localScale.y * 0.5f, position.y - 0.5f);
				}
			}
		}
	}
