...
namespace Rewired.Demos
{
	...
	public class ControlRemappingDemo1 : MonoBehaviour
	{
		...
		private void DrawMouseAssignment()
		{
			...
			bool flag2 = GUILayout.Toggle(flag, "Assign Mouse", "Button", GUILayout.ExpandWidth(expand: false));
			if (flag2 != flag)
			{
				if (flag2)
				{
		...
		private void DrawJoystickSelector()
		{
			...
			bool flag2 = GUILayout.Toggle(flag, "None", "Button", GUILayout.ExpandWidth(expand: false));
			if (flag2 != flag)
			{
			...
			if (selectedPlayer != null)
			{
				foreach (Joystick joystick in ReInput.controllers.Joysticks)
				{
					...
					flag2 = GUILayout.Toggle(flag, joystick.name, "Button", GUILayout.ExpandWidth(expand: false));
					if (flag2 != flag)
					{
						EnqueueAction(new JoystickAssignmentChange(selectedPlayer.id, joystick.id, flag2));
					}
				}
			}
			GUILayout.EndHorizontal();
		...
		private void DrawControllerSelector()
		{
			if (selectedPlayer != null)
			{
				...
				bool flag = selectedController.type == ControllerType.Keyboard;
				bool flag2 = GUILayout.Toggle(flag, "Keyboard", "Button", GUILayout.ExpandWidth(expand: false));
				if (flag2 != flag)
				{
					selectedController.Set(0, ControllerType.Keyboard);
					ControllerSelectionChanged();
				...
				flag = (selectedController.type == ControllerType.Mouse);
				flag2 = GUILayout.Toggle(flag, "Mouse", "Button", GUILayout.ExpandWidth(expand: false));
				if (flag2 != flag)
				{
				...
				{
					flag = (selectedController.type == ControllerType.Joystick && selectedController.id == joystick.id);
					flag2 = GUILayout.Toggle(flag, joystick.name, "Button", GUILayout.ExpandWidth(expand: false));
					if (flag2 != flag)
					{
		...
		private void DrawCalibrateButton()
		{
			...
			else if (GUILayout.Button("Calibrate " + controller.name, GUILayout.ExpandWidth(expand: false)))
			{
		...
		private void DrawMapCategories()
		{
			if (selectedPlayer != null && selectedController.hasSelection)
			{
				...
				foreach (InputMapCategory userAssignableMapCategory in ReInput.mapping.UserAssignableMapCategories)
				{
					...
					bool flag = (userAssignableMapCategory.id == selectedMapCategoryId) ? true : false;
					bool flag2 = GUILayout.Toggle(flag, (!(userAssignableMapCategory.descriptiveName != string.Empty)) ? userAssignableMapCategory.name : userAssignableMapCategory.descriptiveName, "Button", GUILayout.ExpandWidth(expand: false));
					if (flag2 != flag)
					{
						selectedMapCategoryId = userAssignableMapCategory.id;
						selectedMap = selectedPlayer.controllers.maps.GetFirstMapInCategory(selectedController.type, selectedController.id, userAssignableMapCategory.id);
		...
		private void DrawCategoryActions()
		{
			...
			if (actionCategory != null)
			{
				...
				foreach (InputAction item in ReInput.mapping.ActionsInCategory(actionCategory.id))
				{
					...
					else if (item.type == InputActionType.Axis)
					{
		...
		private void DrawElementAssignmentWindow(string title, string message)
		{
			...
			GUILayout.Label("Assignment will be canceled in " + ((int)Mathf.Ceil(num)).ToString() + "...", style_wordWrap);
		}
		...
		private void DrawCalibrationWindow(string title, string message)
		{
			...
			for (int i = 0; i < axisElementIdentifiers.Count; i++)
			{
				...
				bool flag2 = GUILayout.Toggle(flag, controllerElementIdentifier.name, "Button", GUILayout.ExpandWidth(expand: false));
				if (flag != flag2)
				{
			...
			if (calibration.selectedElementIdentifierId >= 0)
			{
				...
				GUILayout.Space(15f);
				bool flag3 = GUILayout.Toggle(axis.enabled, "Enabled", "Button", GUILayout.ExpandWidth(expand: false));
				if (axis.enabled != flag3)
				{
					axis.enabled = flag3;
				}
				GUILayout.Space(10f);
				bool flag4 = GUILayout.Toggle(calibration.recording, "Record Min/Max", "Button", GUILayout.ExpandWidth(expand: false));
				...
				bool flag5 = GUILayout.Toggle(axis.invert, "Invert", "Button", GUILayout.ExpandWidth(expand: false));
