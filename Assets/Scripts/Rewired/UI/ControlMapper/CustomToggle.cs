...
namespace Rewired.UI.ControlMapper
{
	...
	public class CustomToggle : Toggle, ICustomSelectable, ICancelHandler, IEventSystemHandler
	{
		...
		protected override void DoStateTransition(SelectionState state, bool instant)
		{
			...
			{
				Color disabledHighlightedColor = _disabledHighlightedColor;
				Sprite disabledHighlightedSprite = _disabledHighlightedSprite;
				string disabledHighlightedTrigger = _disabledHighlightedTrigger;
				if (base.gameObject.activeInHierarchy)
				{
					switch (base.transition)
					{
					case Transition.ColorTint:
						StartColorTween(disabledHighlightedColor * base.colors.colorMultiplier, instant);
						break;
					case Transition.SpriteSwap:
						DoSpriteSwap(disabledHighlightedSprite);
						break;
					case Transition.Animation:
						TriggerAnimation(disabledHighlightedTrigger);
						break;
					}
				}
			}
